...
class Email extends Persistencia{
	const ASSINATURA = "Atenciosamente,<br/>Suporte da Femeju<br/><a href='http://femeju.com.br'><img src='#url#/img/logo.png' width='120'/></a>";			
	var $assunto;
	var $conteudo;
	var $tipo;
...
function enviarEmailRedefinirSenha($nome, $email,$idUsuario){
		$mensagem = "Sr(a). $nome, sua senha foi redefinida. Clique <a href='".$_SERVER['HTTP_HOST'] . "/admin_usuario-ativar?id=" . $this -> md5_encrypt($idUsuario)."'>Aqui</a> para gerar uma nova senha.";
...
function enviarEmailNovoUsuario($nome, $email,$idUsuario){
        $mensagem = "Sr(a). $nome, você foi cadastrado como usuário do sistema FEMEJU. Clique <a href='".$_SERVER['HTTP_HOST'] . "/admin_usuario-ativar?id=" . $this -> md5_encrypt($idUsuario)."'>Aqui</a> para ativar seu usuário.";
        $tplEmail = new Template("templates/padrao/email.html");
        $tplEmail -> ASSINATURA = Email::ASSINATURA;
        $tplEmail -> MENSAGEM = $mensagem;
